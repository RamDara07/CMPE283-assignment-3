...
u32 exits_count[75];
EXPORT_SYMBOL(numof_exits_count);

u64 exits_cycles[75];
EXPORT_SYMBOL(total_exits_cycles);
...
int kvm_emulate_cpuid(struct kvm_vcpu *vcpu)
{
    ...
    if(eax == 0x4FFFFFFE) {
        ...
        if (kvm_not_defined(ecx)) {
            ...
            printk(KERN_INFO "CPUID(0x4FFFFFFF) Total Cycles for EXIT(%u): %lld",ecx, total_exits_cycles[ecx]);
            exits_i = ecx;
            ecx = total_exits_cycles[exits_i] & 0xFFFFFFFF;
            ebx = (total_exits_cycles[exits_i] >> 32) & 0xFFFFFFFF;
            printk(KERN_INFO "CPUID(0x4FFFFFFF) Total Cycles for EXIT(%u): %lld\n\tebx=%u & ecx=%u", exits_i, total_exits_cycles[exits_i], ebx, ecx);
        }
    }
    else {
